...
def scrap_profile(ids):
    ...
    for id in ids:
        ...
        try:
            event_title = driver.find_element_by_xpath("//h1[@data-testid='event-permalink-event-name']").text
        except:
            event_title = ""
        try:
            organizer = driver.find_element_by_xpath("//div[@data-testid='event_permalink_feature_line']").text
        except:
            try:
                organizer = driver.find_element_by_xpath("//div[@class='_5gnb']/a[@href]").text
            except:
                organizer = ""
        try:
            event_date = driver.find_element_by_xpath("//div[@class='_2ycp _5xhk']").get_attribute("content")
        except:
            event_date = ""
        try:
            attendance = driver.find_element_by_xpath("//a[@class='_5z74']").text
        except:
            attendance = ""
        try:
            for i in driver.find_elements_by_xpath("//ul[@class='_63er']/li"):
                categories += i.text + " "
        except:
            categories = ""
        try:
            description = driver.find_element_by_xpath("//div[@data-testid='event-permalink-details']/span").text
        except:
            description = ""

        df = df.append(
            {'Description': description, 'Categories': categories, 'Attendance': attendance, 'Date': event_date,
             'Organizer': organizer, 'Title': event_title}, ignore_index=True)
